...
BYTE CTPlayer::CheckQuestCondition( LPQUESTTEMP pQUEST, DWORD dwTick, BYTE& bLevel)
{
	...
	for( int i=0; i<INT(pQUEST->m_vCondition.size()); i++)
	{
		...
		case QCT_COUNTRY:
			{
				if(pQUEST->m_vCondition[i]->m_dwConditionID == m_bCountry)
					bResult = QCT_NONE;
				else
					bResult = QCT_COUNTRY;
			}
	...
	case PCT_COUNTRY:
		{
			...
			if(pPortal->m_bCountry != TCONTRY_N)
				return	(m_bCountry == pPortal->m_bCountry ) &&
...
BYTE CTPlayer::CanDuel(CTPlayer * pAttacker)
{
	...
	{ 
		...
		{
			if(m_bCountry == pAttacker->m_bCountry ||
	...
	case CHAT_NEAR:
		{
			...
			if(m_bCountry == GetAttackCountry(bCountry, bCountry))
				return TRUE;
		}
		break;
	case CHAT_WHISPER:
