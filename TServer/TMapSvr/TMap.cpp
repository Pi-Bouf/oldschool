					  ...
					  WORD wUnitID)
{
	...
	for( auto i=0; i<INT(m_vTCHANNEL.size()); i++)
	{
...
//				pMON->OnEvent( AT_TIMEOUT, 0, 0, 0, 0);
	...
	for( auto i=0; i<INT(pSPAWN->m_vMAPMON.size()); i++)
					...
					FLOAT fPosZ)
{
	...
	if( dwOLD != dwNEW )
	{
		...
		if( finder != m_mapTCELL.end() && (*finder).second->IsEnable(pMON->m_bChannel) )
		{
			...
			for( auto i=0; i<BYTE(m_vTCHANNEL.size()); i++)
