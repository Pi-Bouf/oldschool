...
DWORD CTMapSvrModule::OnCS_DEFEND_REQ( LPPACKETBUF pBUF)
{
	...
	if(!pTemp->m_bRunFromServer)
	{
		if(pATTACK)
		{
			...
			bAttackAidCountry = pATTACK->m_bCountry;
	...
	if( pDEFEND )
	{
		...
		{	// 마법거울
			...
			bAttackAidCountry = pDEFEND->m_bCountry;
...
DWORD CTMapSvrModule::OnCS_NPCTALK_REQ(LPPACKETBUF pBUF)
{
	...
	if( !pNpc->CanTalk(pPlayer->m_bCountry, pPlayer->m_bCountry, pPlayer->HaveDisguiseBuff()))
		return EC_NOERROR;
	...
	case CHAT_NEAR:
		{
			...
			{
				CTPlayer * pCHAR = vPLAYERS.back();
				if(pPlayer->CanTalk(bGroup, pCHAR->m_bCountry, pCHAR->m_bAidCountry))
	...
	if(pOBJ)
	{
		...
		if(pOBJ->m_wLocalID != wLocalID)
		{
			...
			if(bType == OT_PC)
			{
				...
				if(pLocal)
				{
					if(pLocal->m_bCountry == pOBJ->m_bCountry)
					{
...
DWORD CTMapSvrModule::OnCS_STOREITEMLIST_REQ(LPPACKETBUF pBUF)
{
	...
	if(pPlayer->m_bCountry != pTarget->m_bCountry)
		return EC_NOERROR;
	...
	if(pPlayer->m_bMain)
	{
		if(pPlayer->m_pMAP)
		{
			...
			while(!vPlayer.empty())
			{
				...
				if(!pChar->CanFight(pPlayer->m_bType, pPlayer->m_bCountry, pPlayer->m_bCountry))
...
DWORD CTMapSvrModule::OnCS_AUCTIONREG_REQ(LPPACKETBUF pBUF)
{
	...
	if(!IsMainCell(DEFAULT_CHANNEL, pPlayer->m_wMapID, pPlayer->m_fPosX, pPlayer->m_fPosZ))
	{
		pPlayer->SendCS_AUCTIONREG_ACK(ABR_AUCTIONNOTFOUND);
		return EC_NOERROR;
	}
...
DWORD CTMapSvrModule::OnCS_AUCTIONINTERESTLIST_REQ(LPPACKETBUF pBUF)
{
	...
	if(!IsMainCell(DEFAULT_CHANNEL, pPlayer->m_wMapID, pPlayer->m_fPosX, pPlayer->m_fPosZ))
	{
		pPlayer->SendCS_AUCTIONINTERESTLIST_ACK(ABR_AUCTIONNOTFOUND);
		return EC_NOERROR;
	}
...
DWORD CTMapSvrModule::OnCS_AUCTIONREGLIST_REQ(LPPACKETBUF pBUF)
{
	...
	if(!IsMainCell(DEFAULT_CHANNEL, pPlayer->m_wMapID, pPlayer->m_fPosX, pPlayer->m_fPosZ))
	{
		pPlayer->SendCS_AUCTIONREGLIST_ACK(ALR_FAIL);
		return EC_NOERROR;
	}
...
DWORD CTMapSvrModule::OnCS_AUCTIONREGCANCEL_REQ(LPPACKETBUF pBUF)
{
	...
	if(!IsMainCell(DEFAULT_CHANNEL, pPlayer->m_wMapID, pPlayer->m_fPosX, pPlayer->m_fPosZ))
	{
		pPlayer->SendCS_AUCTIONREGCANCEL_ACK(ABR_AUCTIONNOTFOUND);
		return EC_NOERROR;
	}
