...
CTObjBase::CTObjBase()
{
	...
	m_bAidCountry = m_bCountry;
	m_bOriCountry = m_bCountry;
	m_bBlock = FALSE;
	...
	while(!vPLAYERS.empty())
	{
		...
		if(m_bType == OT_MON)
		{
			...
			pChar->SendCS_CHANGECOLOR_ACK(
				m_dwID,
				m_bType,
				pMon->GetColor( pChar->m_dwID, pChar->GetPartyID(), pChar->m_bCountry),
				pMon->m_bCountry);
		}
						  ...
						  int nDamageMP)
{
	...
	if(m_bType == OT_PC && nDamageHP > 0 && m_dwHP )
	{
		...
		pMe->CheckSoulHP();
		if(bAttackType != OT_MON && dwHostID != m_dwID && CanFight(bAttackType, bAttackCountry, bAttackCountry))
		{
						...
						BYTE bMirror)
{
		...
		m_pInstanceSkill->m_pTSKILL->IsDamageType())
	{
		...
		if(m_bType == OT_PC)
		{
			...
			if(!CanFight(bAttackType, bAttackCountry, bAttackCountry))
				wAutoSkill = 0;
		}
	...
	if(bAtkHit != HT_MISS && !bMirror)
	{
		pMaintain = MaintainSkill(
			...
			bCurseProb,
			bCanSelect,
			GetAttackCountry(bAttackCountry, bAttackCountry),
			bCP,
			wAL,
			bAttackerLevel,
			fAtkPosX,
				...
				case SDT_LEAD: // 유인
					{
						...
						CTMonster * pMon = (CTMonster *)this;
						if(bAtkerType == OT_PC &&
							CanFight(bAtkerType, bAttackCountry, bAttackCountry))
				...
				case SDT_AGGROTRANS:
					{
						...
						if(dwAggro)
						{
							...
							pMon->AddAggro(dwHostID, pRecall->m_dwID, pRecall->m_bType, pRecall->m_bCountry, dwAggro);
							pMon->LeaveAggro(dwHostID, dwAttack, bAtkerType);
						}
					}
				}
...
{
	if(GetAttackCountry(bCountry, bCountry) == bCountry)
		return FALSE;
