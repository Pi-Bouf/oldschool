...
DWORD CTMapSvrModule::OnMW_ENTERCHAR_REQ( LPPACKETBUF pBUF)
{
	...
	for( BYTE i=0; i<bCount; i++)
	{
		...
		if(pTemp)
		{
			...
			pMON->m_bAidCountry = pPlayer->m_bCountry;
					...
					for( DWORD j=0; j<pSpawn->m_vTMON.size(); j++)
					{
						...
						while(!vPLAYERS.empty())
						{
							...
							pChar->SendCS_CHANGECOLOR_ACK(
								...
								OT_MON,
								pSpawn->m_vTMON[j]->GetColor( pChar->m_dwID, pChar->GetPartyID(), pChar->m_bCountry),
								bCountry);
...
DWORD CTMapSvrModule::OnDM_LOADCHAR_ACK( LPPACKETBUF pBUF)
{
	...
	if(!SetTutorialCharBase(pPlayer))
	{
		...
		if( finder != m_mapTSPAWNPOS.end() &&
			(*finder).second->m_wMapID == TUTORIAL_MAPID) //n
		{
			if(pPlayer->m_pMAP)
				ExitMAP(pPlayer);
	...
	if(pPlayer->m_bMain)
	{
		...
		if(pPlayer->m_pMAP)
		{
			...
			if(wPrevPartyID && !wPartyID)
			{
				...
				{
					WORD wSpawn = GetIndunOutPos(pPlayer->m_wMapID, pPlayer->m_bCountry);
					if(wSpawn)
