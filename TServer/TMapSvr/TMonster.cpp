...
BYTE CTMonster::GetColor(DWORD dwCharID, DWORD dwPartyID, BYTE bCountry)
{
	...
	if(m_bCountry == bCountry)
		return TNCOLOR_ALLI;
	else if(m_bCountry != TCONTRY_N)
			...
			dwTotal >= GetMaxHP()*MONKEEP_PER/100)
		{
			...
			for( DWORD i=0; i<vPlayers.size(); i++)
			{
				vPlayers[i]->SendCS_CHANGECOLOR_ACK(
					m_dwID,
					m_bType,
					GetColor( vPlayers[i]->m_dwID, vPlayers[i]->GetPartyID(), vPlayers[i]->m_bCountry),
					m_bCountry);
			}
		...
		case OWNER_PARTY	:
			{
				...
				for( auto i=0; i<DWORD(wTotalMember); i++)
				{
		...
		while(!vPLAYERS.empty())
		{
			...
			if(bKeepChange)
			{
				pChar->SendCS_CHANGECOLOR_ACK(
					m_dwID,
					OT_MON,
					GetColor( pChar->m_dwID, pChar->GetPartyID(), pChar->m_bCountry),
					m_bCountry);
			}
