...
// Arab Ver.
BOOL CTNationOption::EGYPT = FALSE;
...
// Arab Ver.
BOOL CTNationOption::FLIP_SCREEN = FALSE;
...
void CTNationOption::Clear()
{
	...
	// Arab Ver.
	CTNationOption::FLIP_SCREEN = FALSE;
...
void CTNationOption::SetNation( INT nNation )
{
	...
	case TNATION_GERMANY:
		...
		CTNationOption::INSTALL_HACKSHIELD = FALSE;
		CTNationOption::USE_WEB_CASHSHOP = FALSE;
		THttpCtrl::m_bUnDXRender = TRUE;
		break;
		// Arab Ver.
	case TNATION_EGYPT:
		CTNationOption::EGYPT = TRUE;
		CTNationOption::MODIFY_WORD_BREAK = TRUE;
		CTNationOption::CAMERA_QUAKE = TRUE;
		CTNationOption::INSTALL_HACKSHIELD = FALSE;
		CTNationOption::USE_WEB_CASHSHOP = FALSE;
		THttpCtrl::m_bUnDXRender = TRUE;
		CTNationOption::FLIP_SCREEN = TRUE;
		TEdit::m_nCountryFlag = TEdit::COUNTRY_FLAG_EGYPT;
		CD3DFont::m_bStringFormat = StringFormatFlagsDirectionRightToLeft;
		// Arab Ver.
		CTachyonRes::m_bInverseDirection = TRUE;
		break;
	...
	case TNATION_RUSSIA:
		...
		CTNationOption::CAMERA_QUAKE = TRUE;
		CTNationOption::USE_WEB_CASHSHOP = FALSE;
		CTNationOption::INSTALL_GAMEGUARD = FALSE;	// 게임가드 설치.
...
BOOL CTNationOption::SetNation( CString strNation )
{
	...
	// Arab Ver.
	else if( strNation == _T("EGYPT") )
	{
		SetNation( TNATION_EGYPT );
		return TRUE;
	}
