...
//	if( !CTStrFilter::ConvertString( strMSG, TRUE ) )
//		return ;
	...
	if(bGroup == CHAT_NEAR)
	{
		...
		{
			if((*it).second->m_bContryID == bCountry)
				vTarget.push_back((*it).second->m_dwID);
		}

		vMSG << WORD(vTarget.size());

		for(WORD i=0; i<vTarget.size(); i++)
			vMSG << vTarget[i];
...
/*#ifdef TEST_MODE
	CString strFMT;
	strFMT.Format( "SendCS_ADDCORPSENEMY_REQ : dwReporter(%d) dwEnemyID(%d)",
		dwReporterID,
		pTOBJ->m_dwID );
...
/*#ifdef TEST_MODE
	CString strFMT;
	strFMT.Format( "SendCS_DELCORPSENEMY_REQ : dwReporter(%d) dwEnemyID(%d)",
		dwReporterID,
		dwEnemyID );
	CTClientGame::DebugMSG( strFMT );
#endif*/
